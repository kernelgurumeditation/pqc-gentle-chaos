...
static int16_t sample_cbd(int eta, uint8_t *random_bytes, int *byte_offset) {
    ...
    for (int i = 0; i < eta; i++) {
        /* NOTE: This simplified version uses one byte per 2 bits needed.
         * The real ML-KEM implementation packs bits tightly, using only
         * eta * 2 bits per coefficient (64 bytes per polynomial for eta=2). */
...
static void poly_sample_cbd(poly *p, int eta, uint8_t *seed, int nonce) {
    ...
    /* WARNING: Fake randomness for DEMONSTRATION ONLY!
     * Real ML-KEM derives random_bytes = PRF(seed, nonce) using SHAKE256.
     * This XOR-based substitute is NOT cryptographically secure. */
    for (int i = 0; i < 512; i++) {
...
static void poly_mul(poly *r, const poly *a, const poly *b) {
    int64_t temp[2 * N] = {0};  /* int64_t: accumulation of N products can overflow int32_t */
...
static void message_encode(poly *r, const uint8_t msg[32]) {
    int16_t scale = (Q + 1) / 2;  /* floor(Q/2) = 1664 for Q=3329 */
