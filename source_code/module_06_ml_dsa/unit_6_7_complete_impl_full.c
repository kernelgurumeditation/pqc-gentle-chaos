...
#define ETA_BITS 4
#define T0_BITS 13
#define SK_BYTES (SEEDBYTES * 2 + 64 + \
                  MLDSA_L * MLDSA_N * ETA_BITS / 8 + \
                  MLDSA_K * MLDSA_N * ETA_BITS / 8 + \
                  MLDSA_K * MLDSA_N * T0_BITS / 8)  /* 4032 = 32(ρ)+32(K)+64(tr)+640(s1)+768(s2)+2496(t0) */
...
/* Simple SHAKE256 simulation - EDUCATIONAL ONLY, NOT SECURE!
 * In production, use: EVP_DigestInit_ex(ctx, EVP_shake256(), NULL)
 * from OpenSSL, or OQS_SHA3_shake256() from liboqs */
...
int mldsa_keygen(mldsa_pk *pk, mldsa_sk *sk, const uint8_t seed[SEEDBYTES]) {
    ...
    /* [pack t1 coefficients into pk_bytes — see Unit 6.6 for encoding details] */
...
int mldsa_sign(mldsa_sig *sig, const uint8_t *msg, size_t msglen,
    ...
    do {
        ...
        uint8_t w1_packed[MLDSA_K * MLDSA_N / 2];  /* Simplified packing */
...
int mldsa_verify(const mldsa_pk *pk, const uint8_t *msg, size_t msglen,
    ...
    uint8_t w1_packed[MLDSA_K * MLDSA_N / 2];
    /* Pack w1_prime: must match signing packing exactly */
    for (int i = 0; i < MLDSA_K; i++) {
        for (int j = 0; j < MLDSA_N / 2; j++) {
            w1_packed[i * (MLDSA_N / 2) + j] =
